// Bismillahirrahmanirahim
// Elhamdulillahirabbulalemin
// Ve salatu ve selamu ala resulina Muhammedin ve alihi ve sahbihi ecmain
// Allah U Ekber, Allah U Ekber, Allah U Ekber, La ilahe illallah
// Subhanallah, Elhamdulillah, Allahu Ekber
// Estağfirullah El-Azim

"use client"

import Carousel from 'react-bootstrap/Carousel';

import { Image } from 'react-bootstrap';
function UncontrolledExample() {
  return (
    <Carousel>
      <Carousel.Item>
        <Image src='https://cdn.webtekno.com/media/cache/content_detail_v2/article/23831/turkiye-deki-yazilim-sektorunu-buyutmek-icin-devletten-onemli-adim-1483725143.jpg'/>
        <Carousel.Caption>
          <h3>Yalçın İnşaat</h3>
          <p>Sağlam temeller, modern yaşamlar</p>
        </Carousel.Caption>
      </Carousel.Item>
      <Carousel.Item>
        <Image src='https://www.braintechnosys.com/blog/wp-content/uploads/2020/02/Here-are-top-reasons-Of-Android-App-Development-To-Different-Industries.jpg' />
        <Carousel.Caption>
          <h3>Konut Projeleri</h3>
          <p>Hayalinizdeki eve anahtar teslim</p>
        </Carousel.Caption>
      </Carousel.Item>
      <Carousel.Item>
        <Image  src='https://www.apple.com/newsroom/images/2024/06/ios-18-makes-iphone-more-personal-capable-and-intelligent-than-ever/article/Apple-WWDC24-iPhone-15-Pro-3-up-240610_big.jpg.medium.jpg'/>
        <Carousel.Caption>
          <h3>Ticari Yapılar</h3>
          <p>İş merkezleri ve plazalar</p>

        </Carousel.Caption>
      </Carousel.Item>

      <Carousel.Item>
        <Image  src='https://www.fsm.ac.in/blog/wp-content/uploads/2022/08/ml-e1610553826718.jpg'/>
        <Carousel.Caption>
          <h3>Altyapı Hizmetleri</h3>
          <p>Yol, kanalizasyon ve çevre düzenlemesi
          </p>
        </Carousel.Caption>
      </Carousel.Item>
    </Carousel>
  );
}

export default UncontrolledExample;

// Suphanallah Subhanallah Subhanallahi Azim ve Bihamdihi
// Elhamdulillah Elhamdulillah Elhamdulillah
// Elhamdulillahirabbulalemin//Subhanallah Subhanallah Subhanallahi Azim ve Bihamdihi
// Elhamdulillah Elhamdulillah Elhamdulillah
// Elhamdulillahirabbilalemin
// La ilahe illAllah Muhammeden abduhu ve resuluhu
// Elhamdulillah Elhamdulillah Elhamdulillah
// Elhamdulillah Elhamdulillah Elhamdulillah
// Elhamdulillahirabbilalemin